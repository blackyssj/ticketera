-- 0009 — crear_orden(). Tres garantías en orden:
--  1. `for update` sobre fase_precio ANTES de contar. Contar y después
--     insertar es una carrera: dos leen "queda 1" y los dos insertan.
--  2. La mesa se toma con UPDATE CONDICIONAL: la condición viaja dentro del
--     update, nunca en un `if` previo. 0 filas = alguien llegó primero.
--  3. Idempotencia por client_key antes de tocar stock.
-- NO se le otorga a anon: la llama una Edge Function con service_role.
