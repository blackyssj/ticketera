-- 0046 — una entrada gratis es gratis de verdad
--
-- El fee salía siempre de `greatest(% del subtotal + fijo, piso)`. Con
-- subtotal 0 eso da 0 sólo mientras el organizador tenga el fijo y el piso en
-- cero, que es la configuración de hoy. En cuanto alguno tenga piso, un
-- evento gratis le cobraría ese piso a quien vino a una entrada sin costo — y
-- las dos páginas comerciales prometen que un evento gratis no cuesta nada.
--
-- Se arregla donde la promesa se puede romper, no donde hoy da la casualidad
-- de que se cumple. El resto de la función es idéntico a 0025.
...
begin
  ...
  v_fee := case when v_sub = 0 then 0 else
    greatest(round(v_sub * v_org_row.fee_pct) + v_org_row.fee_fijo_transaccion, v_org_row.fee_piso)
  end;
