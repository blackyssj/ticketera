-- ============================================================
-- 0025 — crear_orden() recibe la atribución ya resuelta
--
-- El navegador nunca manda un rrpp_id: manda un slug (?r=), y
-- crear-orden/index.ts lo resuelve contra perfiles ACOTANDO por el
-- organizador del evento, porque el mismo slug puede repetirse en otro
-- tenant. Acá solo se agrega el parámetro para que la RPC pueda escribir
-- ese resultado ya resuelto en ordenes.rrpp_id (la columna existe desde
-- 0006; nadie la escribía).
--
-- crear_orden() tenía la firma (uuid, jsonb, jsonb, uuid, text). Agregar un
-- parámetro con `create or replace` deja DOS firmas vivas y PostgREST
-- responde "could not choose a candidate function" — la función queda
-- muerta sin que nada avise (invariante 4 de invariantes.sql lo vigila).
-- Por eso primero se hace `drop function` con la firma vieja completa.
-- `if exists` para que correr esto dos veces no falle: la segunda vez esa
-- firma vieja ya no existe.
-- ============================================================

drop function if exists crear_orden(uuid, jsonb, jsonb, uuid, text);
...
revoke execute on function crear_orden(uuid, jsonb, jsonb, uuid, text, uuid) from anon, public;
